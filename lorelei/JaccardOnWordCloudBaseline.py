...
def _find_jaccard_knn(subject_index, word_cloud_list, k):
    ...
    return TestRandomIndexing._extract_top_k(scored_results, k)
...
def run_k_nearest_jaccard_baseline(input_file, output_file,
    ...
    for i in range(0, find_num):
        knn_indexes = _find_jaccard_knn(i, word_cloud_list, k)
        answer = dict()
...
# run_k_nearest_jaccard_baseline(path+'condensed-objects.json', path+'WCjaccard-10-nn-for-first-10-uuids-FULL-nonindent.txt')